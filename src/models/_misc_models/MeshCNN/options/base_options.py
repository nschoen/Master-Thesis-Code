...
class BaseOptions:
    def __init__(self, ignored_required=False):
        self.parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
        self.initialized = False
        self.ignore_required = ignored_required

    def initialize(self):
        required = not self.ignore_required
        # data params
        self.parser.add_argument('--dataroot', required=required, help='path to meshes (should have subfolders train, test)')
    ...
    def parse(self):
        ...
        # set gpu ids
        # CHANGED: use "device" method instead
        print("torch.cuda.device_count()", torch.cuda.device_count())
        if len(self.opt.gpu_ids) > 0:
            torch.cuda.device(self.opt.gpu_ids[0])
