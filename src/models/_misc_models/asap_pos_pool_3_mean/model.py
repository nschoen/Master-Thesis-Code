...
class CustomEdgeConv(MessagePassing):
    ...
    def message(self, x_i: Tensor, x_j: Tensor) -> Tensor:
        #print("x_i", x_i.size())
        #print("x_j", x_j.size())
        return self.nn(torch.cat([x_j[:, 0:3] - x_i[:, 0:3], x_i[:, 3:]], dim=-1))
...
class CustomEdgeConvWeighted(MessagePassing):
    ...
    def message(self, x_i: Tensor, x_j: Tensor, edge_weight: OptTensor) -> Tensor:
        ...
        return self.nn(torch.cat([x_j[:, 0:3] - x_i[:, 0:3], x_i[:, 3:]], dim=-1)) * edge_weight.view(-1, 1)

    def __repr__(self):
        return '{}(nn={})'.format(self.__class__.__name__, self.nn)

class ASAP(torch.nn.Module):
    def __init__(self, num_classes, num_input_channels, num_layers=10, hidden=64, ratio=0.35, dropout=0.1):
        super(ASAP, self).__init__()
        #self.conv1 = GraphConv(num_input_channels, hidden, aggr='mean')
        self.conv1 = CustomEdgeConv(nn=MLP([num_input_channels, hidden, hidden, hidden]), aggr="mean")
        self.convs = torch.nn.ModuleList()
        self.pools = torch.nn.ModuleList()
        self.convs.extend([
            #GraphConv(hidden, hidden, aggr='mean')
            CustomEdgeConvWeighted(nn=MLP([3 + hidden, hidden, hidden, hidden]), aggr="mean")
...
class Model(BaseModel):
    def get_model(self, num_classes=9):
        ...
        self.model = ASAP(num_classes=num_classes, num_layers=10, num_input_channels=num_input_channels)
