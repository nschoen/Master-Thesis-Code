...
class ASAP(torch.nn.Module):
    def __init__(self, num_classes, num_input_channels, num_layers=10, hidden=64, ratio=0.7, dropout=0.1, class_dropout=0.5,
        ...
        if first_conv_use_abs_pos:
            extra_input_neuros += 3
        self.conv1 = CustomEdgeConv(use_norm=use_norm, nn=MLP([num_input_channels - 3 + extra_input_neuros, hidden, hidden]),
                                    aggr="max",
        ...
        self.convs.extend([
            ...
            CustomEdgeConvWeighted(nn=MLP([convs_extra_inputs + hidden, hidden, hidden]), aggr="max",
        ...
        self.lin1 = Linear((num_layers + 1) * hidden, hidden * 2)
        self.lin2 = Linear(hidden * 2, hidden, num_classes)
