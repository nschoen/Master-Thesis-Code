...
class CustomEdgeConv(MessagePassing):
    ...
    def message(self, x_i: Tensor, x_j: Tensor, edge_attr) -> Tensor:
        if self.use_norm:
            return self.nn(torch.cat([edge_attr, x_j - x_i, x_i], dim=-1))
...
class CustomEdgeConvWeighted(MessagePassing):
    ...
    def message(self, x_i: Tensor, x_j: Tensor, edge_weight: OptTensor) -> Tensor:
        if edge_weight is None:
            return self.nn(torch.cat([x_j - x_i, x_i], dim=-1))
        return self.nn(torch.cat([x_j - x_i, x_i], dim=-1)) * edge_weight.view(-1, 1)
...
class ASAP(torch.nn.Module):
    def __init__(self, num_classes, num_input_channels, num_layers=10, hidden=64, ratio=0.7, dropout=0.1, class_dropout=0.5, use_norm=False):
        ...
        self.conv1 = CustomEdgeConv(use_norm=use_norm, nn=MLP([2 * num_input_channels + extra_input_neuros, hidden, hidden, hidden]), aggr="max")
        self.convs = torch.nn.ModuleList()
        self.pools = torch.nn.ModuleList()
        self.convs.extend([
            #GraphConv(hidden, hidden, aggr='mean')
            CustomEdgeConvWeighted(nn=MLP([2 * hidden, hidden, hidden, hidden]), aggr="max")
