...
class Net(torch.nn.Module):
    def __init__(self, num_features, num_classes):
        ...
        self.conv1 = CustomEdgeConv(nn=MLP([2 * 3 + 5, 32, 32, 64]), aggr="max")
        self.conv2 = CustomEdgeConv(nn=MLP([2 * 3 + 64, 64, 64, 64]), aggr="max")
        self.conv3 = CustomEdgeConv(nn=MLP([2 * 3 + 64, 64, 64, 64]), aggr="max")
        self.conv4 = CustomEdgeConv(nn=MLP([2 * 3 + 64, 64, 64, 64]), aggr="max")
    ...
    def forward(self, data):
        ...
        x2 = self.conv2(x1, pos, edge_index)
        x3 = self.conv3(x2, pos, edge_index)
        x4 = self.conv4(x3, pos, edge_index)
        x = gmp(x1, batch) + gmp(x2, batch) + gmp(x3, batch) + gmp(x4, batch)
