...
class Model(BaseModel):
    def __init__(self, config):
        BaseModel.__init__(self, config, T.ToDense(max_nodes)) # transform

    def get_model(self, num_classes=9):
        data_item = self.test_dataset[0]

        #int(exp['max_number_triangles'] / 2 + 2)

        num_vertex_features = data_item.x.shape[1]
        self.model = Net(max_nodes=max_nodes, n_input_dim=num_vertex_features, num_classes=num_classes)


        # self.model = MODEL.get_
