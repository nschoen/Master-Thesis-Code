...
class Net(torch.nn.Module):
    def __init__(self, out_channels, k=20, aggr='max'):
        ...
        self.conv3 = DynamicEdgeConv(MLP([2 * 64, 128, 512]), k, aggr)
    ...
    def forward(self, data):
        ...
        l1 = global_max_pool(out, batch)

        # select lowest norm
        #x_norm = torch.linalg.norm(x2, dim=0)
        #torch.topk(x, 3)
        ...
        #perm = topk(x2, 0.4, batch, min_score=None, tol=1e-7)
        #x2 = x2[perm]
        #batch = batch[perm]
        x2, batch, _, _ = self.pool1(x2, batch)
        x3 = self.conv3(x2, batch)
        l2 = global_max_pool(x3, batch)

        #out = global_max_pool(out, batch)
        #print("l1.shape", l1.shape)
        #print("l2.shape", l2.shape)
        out = torch.cat((l1, l2), dim=-1)
