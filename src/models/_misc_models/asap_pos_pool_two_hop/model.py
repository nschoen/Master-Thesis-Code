...
class CustomEdgeConv(MessagePassing):
    ...
    propagate_type = {'x': PairTensor} # Optional[Tensor]
...
class CustomEdgeConvWeighted(MessagePassing):
    ...
    def message(self, x_i: Tensor, x_j: Tensor, edge_weight: OptTensor) -> Tensor:
        #print("x_i", x_i.size())
        #print("x_j", x_j.size())
        if edge_weight is None:  # len(edge_weight.size()) == 0
            return self.nn(torch.cat([x_i[:, 0:3], x_j[:, 0:3] - x_i[:, 0:3], x_i[:, 3:]], dim=-1))
...
class ASAP(torch.nn.Module):
    def __init__(self, num_classes, num_input_channels, num_layers=10, hidden=64, ratio=0.7, dropout=0.1):
        ...
        self.conv1 = CustomEdgeConv(nn=MLP([2 * 3 + num_input_channels - 3, hidden, hidden, hidden]), aggr="max").jittable()
        self.convs = torch.nn.ModuleList()
        self.pools = torch.nn.ModuleList()
        self.convs.extend([
            #GraphConv(hidden, hidden, aggr='mean')
            CustomEdgeConvWeighted(nn=MLP([2 * 3 + hidden, hidden, hidden, hidden]), aggr="max").jittable()
    ...
    def forward(self, data):
        ...
        #x, pos, edge_index, batch = data.x, data.pos, data.edge_index, data.batch
        edge_weight = None  # torch.empty(())  # None
        data_2hop = TwoHop()(data)
        x = F.relu(self.conv1(x, pos, data_2hop.edge_index))
