...
class CustomEdgeConv(MessagePassing):
    ...
    def __init__(self, nn: Callable, aggr: str = 'max', use_norm=False, use_absolute_pos=True, normalize_direction=False,
                 use_direction_angle=False, use_abs_direction=False, use_absolute_norm=False, use_x_j_features=False, **kwargs):
        super(CustomEdgeConv, self).__init__(aggr=aggr, **kwargs)
        self.nn = nn
        self.reset_parameters()
        self.use_norm = use_norm
        self.use_absolute_norm = use_absolute_norm
        ...
        self.use_x_j_features = use_x_j_features

    def reset_parameters(self):
        reset(self.nn)

    def forward(self, x: Tensor, pos: Tensor, edge_index: Adj, edge_attr=None, normal=None) -> Tensor:
        """"""
        ...
        #print("row", row)
        #normal_j = normal[row]
        #print("normal_j", normal_j)
        #normal = (normal[row], normal[col])
        return self.propagate(edge_index=edge_index, x=x, edge_attr=edge_attr, normal=normal, size=None)


    def message(self, x_i: Tensor, x_j: Tensor, edge_attr, normal_i, normal_j) -> Tensor:
        if self.use_x_j_features:
            input = x_j[:, 3:]
        else:
            input = x_i[:, 3:]
        ...
        if self.use_absolute_pos:
            if self.use_x_j_features:
                input = torch.cat([x_j[:, 0:3], input], dim=-1)
            else:
                input = torch.cat([x_i[:, 0:3], input], dim=-1)
        if self.use_absolute_norm:
            if self.use_x_j_features:
                input = torch.cat([normal_j, input], dim=-1)
            else:
                input = torch.cat([normal_i, input], dim=-1)

        return self.nn(input)

        # if self.use_norm:
        #     if self.use_absolute_pos:
        #         return self.nn(torch.cat([x_i[:, 0:3], edge_attr, direction, x_i[:, 3:]], dim=-1))
        #     else:
        #         return self.nn(torch.cat([edge_attr, direction, x_i[:, 3:]], dim=-1))
        # if self.use_absolute_pos:
        #     return self.nn(torch.cat([x_i[:, 0:3], direction, x_i[:, 3:]], dim=-1))
...
class ASAP(torch.nn.Module):
    def __init__(self, num_classes, num_input_channels, num_layers=10, hidden=64, ratio=0.7, dropout=0.1, class_dropout=0.5,
                 use_norm=False, use_absolute_norm=False, use_absolute_pos=True, normalize_direction=False, convs_use_direction=True, convs_use_diff=False,
                 first_conv_use_direction_angle=False, first_conv_use_abs_pos=True, first_conv_use_abs_direction=True,
                 first_conv_use_x_j_features=False):
        ...
        if first_conv_use_abs_pos:
            extra_input_neuros += 3
        if use_absolute_norm:
            extra_input_neuros += 3
        self.conv1 = CustomEdgeConv(use_norm=use_norm,
                                    use_absolute_norm=use_absolute_norm,
                                    nn=MLP([num_input_channels - 3 + extra_input_neuros, hidden, hidden, hidden]),
                                    aggr="max",
                                    ...
                                    use_x_j_features=first_conv_use_x_j_features,
                                    )
    ...
    def forward(self, data):
        x, pos, edge_attr, edge_index, batch, normal = data.x, data.pos, data.edge_attr, data.edge_index, data.batch, data.normal
        edge_weight = None
        x = F.relu(self.conv1(x, pos, edge_index, edge_attr, normal))
...
class Model(BaseModel):
    def get_model(self, num_classes=9):
        ...
        use_absolute_normal = self.config.asap_use_absolute_normal if hasattr(self.config, 'asap_use_absolute_normal') else False
        ...
        first_conv_use_x_j_features = self.config.asap_first_conv_use_x_j_features if hasattr(self.config, 'asap_first_conv_use_x_j_features') else False
        ...
        self.model = ASAP(
            ...
            use_absolute_norm=use_absolute_normal,
            first_conv_use_x_j_features=first_conv_use_x_j_features,
        )
