...
class Process:
    def __init__(self, obj_file, target_faces, export_name, triangulate=True):
        mesh = self.load_obj(obj_file)
        #self.merge_close_vertices(mesh)
        self.simplify(mesh, target_faces, triangulate=triangulate)
        #self.remove_zero_area_faces(mesh)
    ...
    def simplify(self, mesh, target_faces, triangulate=True):
        bpy.context.view_layer.objects.active = mesh
        mod = mesh.modifiers.new(name='Decimate', type='DECIMATE')
        bpy.context.object.modifiers['Decimate'].use_collapse_triangulate = triangulate
        #
    ...
    def merge_close_vertices(self, mesh):
        if bpy.ops.object.mode_set.poll():
            bpy.ops.object.mode_set(mode='EDIT')
        bpy.ops.mesh.remove_doubles(threshold=0.00001)
        bpy.ops.object.mode_set(mode='OBJECT')

    def remove_zero_area_faces(self, mesh):
        bpy.context.view_layer.objects.active = mesh
        if bpy.ops.object.mode_set.poll():
            bpy.ops.object.mode_set(mode='EDIT')
