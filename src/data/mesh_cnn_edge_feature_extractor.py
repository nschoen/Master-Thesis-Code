...
def fill_mesh(file: str, opt):
    load_path = get_mesh_path(file, opt.num_aug)

    # depending on if scale_verts is enabled, load the correct cache file
    if not hasattr(opt, 'scale_verts') or not opt.scale_verts:
        load_path = load_path.replace('.npz', '-no-scale.npz')
    ...
    if os.path.exists(load_path):
        ...
        np.savez_compressed(load_path, gemm_edges=mesh_data.gemm_edges, vs=mesh_data.vs, edges=mesh_data.edges,
                            ...
                            features=mesh_data.features, edge_normals=mesh_data.edge_normals)

    if opt.mesh_edge_normals and len(mesh_data['edge_normals']) == 0:
        mesh_data = from_scratch(file, opt)
        np.savez_compressed(load_path, gemm_edges=mesh_data.gemm_edges, vs=mesh_data.vs, edges=mesh_data.edges,
                            edges_count=mesh_data.edges_count, ve=mesh_data.ve, v_mask=mesh_data.v_mask,
                            filename=mesh_data.filename, sides=mesh_data.sides,
    ...
    return mesh_data['gemm_edges'], mesh_data['features'], mesh_data['vs'], mesh_data['edges'], mesh_data['edge_normals']
...
def get_mesh_path(file: str, num_aug: int):
    ...
    if num_aug > 1:
        ...
        #load_file = os.path.join(load_dir, '%s_%03d.npz' % (prefix, 16)) # for debugging use explictely aug number 16
    else:
        load_file = os.path.join(load_dir, '%s_original.npz' % (prefix,))
...
def from_scratch(file, opt):
    ...
    original_vs = mesh_data.vs.copy()
    original_faces = faces.copy()

    mesh_data.v_mask = np.ones(len(mesh_data.vs), dtype=bool)
    faces, face_areas, mask = remove_non_manifolds(mesh_data, faces)
    if opt.num_aug > 1:
    ...
    if opt.mesh_edge_normals:
        mesh_data.edge_normals = get_edge_normals(file, original_vs, mesh_data.vs, mesh_data['edges'])
    else:
        mesh_data.edge_normals = []

    return mesh_data

def get_edge_normals(file, original_vs, augmented_vs, edges):
    '''
    Calcualte edge normals. This function reads the mesh file again with open3d to get the face normals and map
    them to the already existing mesh. This is done to utilize open3d's normal vector estimation where normal vectors
    point in the direction out of the watertight model.
    :param file:
    :param original_vs: vertices coordinates before augmentation to match id with open3d's triangular mesh vertices
    :param augmented_vs:
    :param edges:
    :return:
    '''
    ...
    pcd.points = o3d.utility.Vector3dVector(o3d_vertices)
    kd_tree = o3d.geometry.KDTreeFlann(pcd)

    # find mapping of mesh vertex IDs to o3d mesh vertex IDs
    vs_id_map = []
    for j in range(len(original_vs)):
        vertex = original_vs[j]
        idx = kd_tree.search_knn_vector_3d(vertex, 1)[1][0]
        vs_id_map.append(idx)

    # set new augmented_vs as vertices and compute vertex normals
    for i, augmented_vs in enumerate(augmented_vs):
        o3d_vertices[vs_id_map[i]] = augmented_vs
    mesh.vertices = o3d.utility.Vector3dVector(o3d_vertices)
    mesh = mesh.compute_vertex_normals()
    mesh = mesh.compute_triangle_normals()

    # visualize during debugging
    ...
    # collect adjacent face normals by edge IDs
    edge_face_normals = {}
    edge_normals = {}
    edge_normal_done = {}
    i = 0
    for t in triangles:
        ...
        v1 = t[1]
        v2 = t[2]
        for v_a, v_b in [[v0, v1], [v1, v2], [v0, v2]]:
            edge_id = '-'.join([str(x) for x in sorted([v_a, v_b])])
            edge_normal_done[edge_id] = False
            if not edge_id in edge_face_normals:
                edge_face_normals[edge_id] = []
            edge_face_normals[edge_id].append(triangle_normals[i])
        i += 1

    # calculate edge normals
    num_edge_cases = 0
    for edge_id, face_normals in edge_face_normals.items():
        if len(face_normals) == 2:
            # both faces provided, calculate edge normal dependent on both
            normal1 = face_normals[0]
            normal2 = face_normals[1]
            edge_normal = (normal1 + normal2) / 2
            if np.linalg.norm(edge_normal) == 0.0:
                edge_normal = np.array([0.0, 1.0, 0.0])
            else:
                edge_normal = edge_normal / np.linalg.norm(edge_normal)
            edge_normals[edge_id] = edge_normal
            edge_normal_done[edge_id] = True
        else:
            # edge case, edge only has one adjacent face, just take the face's normal
            normal = face_normals[0]
            edge_normals[edge_id] = normal / np.linalg.norm(normal)
            num_edge_cases += 1
    # if num_edge_cases > 0:
    #     print("num edges cases", num_edge_cases)

    # map edge normals from o3d Mesh back to MeshCNNs mesh
    final_edge_normals = []
    for i, edge in enumerate(edges):
        edge_id_a = vs_id_map[edge[0]]
        edge_id_b = vs_id_map[edge[1]]
        edge_id = '-'.join([str(x) for x in sorted([edge_id_a, edge_id_b])])
        if edge_id in edge_normals:
            final_edge_normals.append(edge_normals[edge_id])
        else:
            final_edge_normals.append(np.array([1.0, 0.0, 0.0]))

    return np.array(final_edge_normals)
    ...
    for face_id, face in enumerate(faces):
        ...
        if is_manifold:
            ...
            for idx, edge in enumerate(faces_edges):
                edges_set.add(edge)
    return faces[mask], face_areas[mask], mask
...
def augmentation(mesh, opt, faces=None):
    if hasattr(opt, 'scale_verts') and opt.scale_verts:
        scale_verts(mesh)
    if hasattr(opt, 'flip_edges') and opt.flip_edges and opt.flip_edges > 0.0:
        faces = flip_edges(mesh, opt.flip_edges, faces)
    return faces


def post_augmentation(mesh, opt):
    if hasattr(opt, 'slide_verts') and opt.slide_verts and opt.slide_verts > 0.0:
...
def slide_verts(mesh, prct):
    ...
    for vi in vids:
        if shifted < target:
            edges = mesh.ve[vi]
            # try:
            if len(dihedral[edges]) == 0:  # CUSTOM
                continue
